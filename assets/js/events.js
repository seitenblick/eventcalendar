...
function buildrruleset(thisobj) {
    ...
    if(allEventsResult==''){
        ...
        else if (mode === 'event-slider'){
            ruleelement.append("            <div class=\"event-slider slick-events\">\n" +
                "              <div class=\"nextevents-slide\"><div class=\"next-events\">Nächste Veranstaltungen</div></div></div><a href=\"https://www.aalen-tourismus.de/kalender.5833.255.htm\" class=\"events-all\">Alle Veranstaltungen</a>\n" +
                "      <a href=\"https://www.aalen-tourismus.de/highlights.63736.255.htm\" class=\"events-highlights pull-right\">Veranstaltungshighlights anzeigen</a>");
        }
        else if (mode === 'next-event-list'){
            ruleelement.append("<div class=\"next-events-entries\"></div>");
        }
        else if (mode === 'eventTable'){
            var template = Hbs['eventTable'];
        ...
        $.each(allEventsResult, function( key, item, element=ruleelement ) {
            ...
            if (mode === 'list') {
                ...
                element.find('.event-widget .row').append('            <div class="col-12 col-sm-4">\n' +
                    '                <a href="' + item.url +'" class="event-item">\n' +
                    '                    <div class="event-image">\n' +
                    '                  <img\n' +
                    ' src="' + item.image.thumb_300px +'"\n' +
                    ' class="lazyload img-responsive"\n' +
                    ' title="' + item.image.titleTag + '"\n' +
                    ' alt="' + item.image.altTag + '"\n' +
                    ' data-sizes="auto"\n' +
                ...
                var compareStart = DateTime.fromJSDate(item.startdate, {zone:'UTC'}).toFormat('DD');
                var compareEnd = DateTime.fromJSDate(item.enddate, {zone:'UTC'}).toFormat('DD');
                element.find('.event-slider').append(' <div>\n' +
                    '                <a href="' + item.url +'">\n' +
                    '                  <div class="event-date">' + DateTime.fromJSDate(itemDate).setLocale(lang).toFormat('ccc. dd. MM') + '</div>\n' +
                    '                  <img\n' +
                    ' src="' + item.image.thumb_100px +'"\n' +
                    ' class="lazyload img-responsive"\n' +
                    ' title="' + item.image.titleTag + '"\n' +
                    ...
                    '                    <div class="headline">' + item.title + '</div>\n' +
                    '                    <div class="slider-caption">\n' +
                    '                        <p>\n' + `${item.timeValid ? '<span class="event-time"><time>' + itemHour + ':' + itemMinutes + ' Uhr</time></span>' : ''}<br />` +
                    item.location +
                    '                        </p>\n' +
                    '                    </div>\n' +
                    '                  </div>\n' +
                ...
                element.find('.next-events-entries').append('<div class="next-event-entry">\n' +
                    '              <a href="' + item.url + '" tabindex="0" class="">\n' +
                    '                   <span class="next-event-day">' + DateTime.fromJSDate(itemDate).setLocale(lang).toFormat('ccc. dd. MM') + '</span>\n' +
                    '                   <span class="next-event-title">' + truncate(item.title,18) + '</span>\n' +
                    '              </a>\n' +
                    '           </div>');
            }
            else if (mode === 'eventTable'){
                //No output for handleBars eventTable
            }
            else {
