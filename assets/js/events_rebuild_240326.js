...
function contentfiltervalue(filterValue){
    ...
    if (gesuchteKategorien != '') {
        ...
        // var gefiltertesArray = meinArray.filter(function(element) {
        //     var namenListe = element.category.id.split(',').map(id => id.trim()); // Trenne die Namen und entferne Leerzeichen
        //     return gesuchteNamen.every(gesuchterName => namenListe.includes(gesuchterName)); // Überprüfe, ob jeder gesuchte Name in der Namensliste enthalten ist
        // });

        // var gefiltertesArray = meinArray.filter(function(element) {
        //     return element.category.id.includes(gesuchterName);
        // });
...
$(document).ready(function() {
    ...
    $('[data-filter]').on('click', function () {
        ...
        /* eingefügt, damit das Array neu erzeugt wird, da es u.U. sonst nicht passt. Sollte perspektivisch raus.*/
        //Termine erzeugen und darstellen
        // buildrruleset($('.rruleset'));
        /**/
    ...
    const flatpickrRange = $(".flatpickr-range").flatpickr({
        ...
        onChange: function(selectedDates, dateStr, instance,events,allEvents) {
            if(selectedDates=='') {
                ...
                //initCalendar(url, $('.rruleset'));
                // allEventsResult = eventsResultRangeFiltered;
                // allEventsResult = initalEventsResult;
                //eventsResultRangeFiltered = initalEventsResult;
                // buildrruleset($('.rruleset'));
...
//Clear Flatpickr if clear-button is clicked
    $('.flatpickr-group .clear').on('click', function(){
        ...
        //     }, 500);
        // initCalendar(url, $('.rruleset'));
        // allEventsResult = allEventsResultInit;
        // allEventsResult = initalEventsResult;
        // eventsResultRangeFiltered = initalEventsResult;

        // initCalendar(url, $('.rruleset'));
        // eventsResultRangeFiltered = restoreArray;
        // console.log("Clear fp: allEventsResult:");
        // console.log(allEventsResult);
        // alert(allEventsResult.length);
        // eventfilter();
        // printEventsHandlebars();
